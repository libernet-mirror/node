...
pub fn recover_ed25519_public_key(
    ...
    let bytes = ed25519_dalek::pkcs8::PublicKeyBytes::from_public_key_der(bytes)
        .map_err(|_| rustls::Error::InvalidCertificate(rustls::CertificateError::BadEncoding))?;
...
mod tests {
    ...
    fn test_certificate_generation() {
        ...
        let ed25519_public_key = utils::compress_point_25519(account.ed25519_public_key());
        let ed25519_public_key_der = {
            let bytes = ed25519_dalek::pkcs8::PublicKeyBytes(ed25519_public_key.to_fixed_bytes());
            bytes.to_public_key_der().unwrap().to_der().unwrap()
        };
        let certificate =
        ...
        assert_eq!(
            ...
            x509_parser::public_key::PublicKey::Unknown(ed25519_public_key_der.as_slice())
        );
        assert_eq!(
    ...
    // This is atrocious but seems to be the only way to make the custom rustls configs work. The
    // damn rustls won't accept an external signing object.
